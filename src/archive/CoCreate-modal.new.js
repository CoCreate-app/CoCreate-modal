...
function CoCreateModal(el, options) {
  ...
  /** define constant **/	
	...
	this.id = "userWin-" + CoCreateWindow.generateUUID(10);
	this.el = el;
	this.clicked = null;
	this.redraw = false;
	...
	localStorage.setItem("windowId", this.id)
...
CoCreateModal.prototype = {
  ...
  togglePark: function() {
    ...
    console.log('changed parked status')
  },
...
function CoCreateWindow(id) {
  ...
  this.isParent = config.isParent ? config.isParent : false;
  
  if (!this.isParent) {
    this.parentId = localStorage.getItem('parentId');
    this.windowId = localStorage.getItem('windowId');  
  }
...
CoCreateWindow.prototype = {
  ...
  _initSocket: function() {
    ...
    CoCreateSocket.listen('windowBtnEvent', function(data) {
      if (data.parentId == _this.pageId) {
        ...
        var wndId = data.windowId;
        var type = data.type;
        
        var modal = _this.container.getModalById(wndId);
  ...
  setParent: function() {
  	localStorage.setItem('parentId', this.pageId);
  }
}
...
var g_cocreateWindow = new CoCreateWindow("windows");